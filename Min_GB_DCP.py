...
def estimate_A_GB(dark_channel,DCP_input_img):
...
def estimate_transmission_map_GB(A,DCP_input_img):
    ...
    _,dark_channel = get_dark_channel_GB(normalized_img)
    w = 0.95

    return np.ones(dark_channel.shape) - w*dark_channel

def get_scene_radiance_GB(DCP_input_img):
    ...
    _,dark_channel = get_dark_channel_GB(DCP_input_img_normalized)
    A = estimate_A_GB(dark_channel=dark_channel,DCP_input_img=DCP_input_img_normalized)
    t_map = estimate_transmission_map_GB(A,DCP_input_img_normalized)
...
if __name__ == "__main__":
    ...
    for i in range(len(raw_imgs)):
        DCP_input_img = raw_imgs[i]
        min_channel,dark_channel = get_dark_channel_GB(DCP_input_img)

        output_DCP = get_scene_radiance(DCP_input_img) 

        output_GB = get_scene_radiance_GB(DCP_input_img)
        ...
        ax[1].imshow(output_DCP)
        ax[1].set_title("Normal DCP")
        ax[1].axis("off")
